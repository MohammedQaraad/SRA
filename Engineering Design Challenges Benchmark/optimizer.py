...
def selector(algo, func_details, popSize, Iter):
    ...
    ub = func_details[2]
    dim = func_details[3]
    fun_id= func_details[4]


    if algo == "SRA":
        x = sra.SRA(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter,fun_id) 
                    ...
                    with open(ExportToFile, "a", newline="\n") as out:
                        ...
                        if (
                            ...
                            header = numpy.concatenate(
                                [["Optimizer", "objfname", "ExecutionTime", "BestX"], CnvgHeader]
                            )
                            writer.writerow(header)
                            Flag_details = True  # at least one experiment
                        executionTime[k] = x.executionTime
                        a = numpy.concatenate(
                            [[x.optimizer, x.objfname, x.executionTime, x.BestX], x.convergence]
                        )
                        writer.writerow(a)
                    out.close()
